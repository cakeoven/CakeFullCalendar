...
<div class="actions">
	<ul>
		...
		<li><?php echo $this->Html->link(__('Delete Event Type', true),
				...
				sprintf(__('Are you sure you want to delete %s?', true), $eventType['EventType']['name'])); ?> </li>
				...
				<tr<?php echo $class; ?>>
					...
					<td class="actions">
						<?php echo $this->Html->link(__('View', true),
							['plugin' => 'calendar', 'controller' => 'events', 'action' => 'view', $event['id']]); ?>
						<?php echo $this->Html->link(__('Edit', true),
							['plugin' => 'calendar', 'controller' => 'events', 'action' => 'edit', $event['id']]); ?>
					</td>
				</tr>
			<?php endforeach; ?>
		</table>
